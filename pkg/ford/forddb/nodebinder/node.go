...
func (w *_node) LookupByString(key string) (datamodel.Node, error) {
	...
	case *schema.TypeStruct:
		...
		fval := nonPtrVal(w.val).FieldByName(globalFieldNameCache.Get(w.val.Type(), key).Name)
		if !fval.IsValid() {
...
func (w *_assembler) BeginMap(sizeHint int64) (datamodel.MapAssembler, error) {
	...
	case *schema.TypeStruct:
		...
		return &_structAssembler{
			cfg:           w.cfg,
			schemaType:    typ,
			fieldMappings: globalFieldNameCache.GetAll(val.Type()),
			val:           val,
...
type _structAssembler struct {
	...
	fieldMappings map[string]reflect.StructField
