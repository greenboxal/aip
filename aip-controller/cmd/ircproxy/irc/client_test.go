...
type TestHandler struct {
	messages []*irc2.Message
	delay    time.Duration
}

func (th *TestHandler) Handle(c *irc2.Client, m *irc2.Message) {
...
func (th *TestHandler) Messages() []*irc2.Message {
	ret := th.messages
	th.messages = nil
	return ret
}
...
func TestCapReq(t *testing.T) {
	...
	// Happy path
	c := runClientTest(t, config, io.EOF, func(c *irc2.Client) {
	...
	c = runClientTest(t, config, errors.New("CAP multi-prefix requested but was rejected"), func(c *irc2.Client) {
		assert.False(t, c.CapAvailable("random-thing"))
		assert.False(t, c.CapAvailable("multi-prefix"))
		c.CapRequest("multi-prefix", true)
	}, []TestAction{
	...
	c = runClientTest(t, config, errors.New("CAP multi-prefix requested but not accepted"), func(c *irc2.Client) {
		assert.False(t, c.CapAvailable("random-thing"))
		assert.False(t, c.CapAvailable("multi-prefix"))
		c.CapRequest("multi-prefix", true)
	}, []TestAction{
...
func TestClientHandler(t *testing.T) {
	...
	assert.EqualValues(t, []*irc2.Message{
		{
			Tags:    irc2.Tags{},
			Prefix:  &irc2.Prefix{},
			Command: "001",
...
func TestFromChannel(t *testing.T) {
	...
	c := irc2.NewClient(newNopCloser(&bytes.Buffer{}), irc2.ClientConfig{Nick: "test_nick"})

	m := irc2.MustParseMessage("PRIVMSG test_nick :hello world")
	assert.False(t, c.FromChannel(m))

	m = irc2.MustParseMessage("PRIVMSG #a_channel :hello world")
	assert.True(t, c.FromChannel(m))

	m = irc2.MustParseMessage("PING")
	assert.False(t, c.FromChannel(m))
}
