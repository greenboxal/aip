...
func testReadMessage(t *testing.T, c *irc2.Conn) *irc2.Message {
	t.Helper()
...
func TestWriteMessageError(t *testing.T) {
	...
	err := c.WriteMessage(irc2.MustParseMessage("PING :hello world"))
	assert.Error(t, err)
...
func TestConn(t *testing.T) {
	...
	m := &irc2.Message{Prefix: &irc2.Prefix{}, Command: "PING", Params: []string{"Hello World"}}
	...
	m = irc2.MustParseMessage("PONG :Hello World")
	...
	m = irc2.MustParseMessage("001 test_nick")
	rwc.server.WriteString(m.String() + "\r\n")
	m2 = testReadMessage(t, c)
	assert.EqualValues(t, m, m2, "Message returned by client did not match input")
	...
	_, err = c.ReadMessage()
	assert.Equal(t, irc2.ErrMissingDataAfterPrefix, err)
